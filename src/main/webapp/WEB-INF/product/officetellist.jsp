...
<body>
			...
			<!-- 검색 옵션 -->
				...
				<div class="search-option3">
				 	 ...
				 	 <c:forEach var="i" begin="1" end="25">
				 		<img id="gu${i }" src="../resources/img/map/gu_${i }_off.png" class="images" data-value="${i }" @click="mapFindofficetel(${i })">
				 	 </c:forEach>
				</div>		
				<div class="search-option1">
			     	<input type=text size=20 class="input-sm" id="searchfd" style="float: left" v-model="fd" value="${fd }" @keyup.enter="findofficetel()">
			        <input type=button value="검색" id="searchbtn"  class="btn btn-sm btn-danger" @click="findofficetel()">
			    </div>
	...
	<!--정렬 -->
				...
				<div class="row">
					<table class="table">
						<tr id = "tableup">
							...
							<tr id="tabledown" v-for="(vo,index) in officetel_list">
								...
								<td width=20% class="text-center">
											<a :href="'../product/officetelDetail.do?no='+vo.no">{{vo.name }}</a>
								</td>							
					...
					<nav class="pagination">
						<ul>
							<li v-if="startPage!=1"><a @click="prev()">&laquo;</a></li>
								<li v-for="i in pageList" :class="i==curpage?{currentRed: isActive}:{currentRed:false}" ><a @click="officetellistData(i)">{{i }}</a></li>
	    ...
	    new Vue({
	    	...
	    	mounted:function(){
	    		this.officetellistData(1);
	    	},
	    	methods:{
	    		officetellistData:function(page){
	    			...
	    			axios.post("http://localhost:8080/web/product/officetellist_vue.do",JSON.stringify(data),{
	    				headers: {
		                    "Content-Type": "application/json;charset=utf-8"
		                },
	    			}).then(function(res){
	    				console.log(res)
	    				_this.officetel_list=res.data;
	    		...
	    		prev:function(){
	    			...
	    			this.officetellistData(this.curpage);
	    		},
	    		next:function(){
